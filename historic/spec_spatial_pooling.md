...
### 12.25 Débogage de 2021 — quatre hypothèses réfutées, puis la bonne

Après les correctifs de données de §12.21 et §12.24, le modèle joint ne
converge toujours pas sur les grandes coupures. Bilan des cinq coupures, roster
as-of, niveau/dérive séparés :

| coupure | nœuds | M | cand. | occ. max | K | R-hat | ESS | div | pas |
|---|---|---|---|---|---|---|---|---|---|
| J-300 | 36 | 12 | 15 | 3 | 9 | 1,070 | 51,7 | 10 | 255 |
| J-240 | 56 | 18 | 16 | 4 | 13 | 1,053 | 76,1 | 10 | 255 |
| J-210 | 86 | 24 | 19 | 5 | 15 | **2,263** | 2,8 | 80 | 511 |
| J-180 | 151 | 34 | 20 | 5 | 15 | **2,129** | 2,6 | 14 | 511 |
| J-150 | 195 | 44 | 21 | 6 | 15 | **2,556** | 2,4 | 1 | 511 |

Discontinuité franche entre J-240 et J-210. Un ESS de 2,4 à 2,8 pour 1600
tirages ne veut pas dire « mélange lent » : ça veut dire que les quatre chaînes
valent quatre points, donc des modes distincts.

**Quatre hypothèses, toutes fausses.** Elles sont notées parce qu'elles
coûteraient le même temps à qui les reprendrait :

1. *Occupation des blocs.* Plausible (2027 curé à 3 candidats par groupe max,
   2022 générique à 6), mais **J-210 à occupation 5 est pire que J-150 à
   occupation 6**. Pas monotone.
2. *Dérive réelle incompatible avec le noyau OU* — Zemmour triplant à l'automne
   2021. Mesuré, en écarts-types de la dérive autorisée
   `sigma_w·sqrt(2(1-e^{-T/tau}))` : **aucun candidat, aucune coupure, au-delà
   de 2 σ**. Zemmour à J-150 est le maximum à 1,5 σ, et J-210 — la pire
   coupure — a les dérives les PLUS FAIBLES (0,8 σ). Réfutée.
3. *Saturation de la troncature KL.* Les trois coupures qui échouent sont
   exactement celles où `K` atteint le plafond `k_max = 15` — coïncidence
   frappante. Mais à J-210 ces 15 composantes capturent **99,0 %** de la
   variance, soit exactement la cible ; seule J-150 descend à 97,2 %. La
   corrélation est de la colinéarité avec la taille du problème. Réfutée.
4. *Petits candidats.* Le pire désaccord entre chaînes à J-210 portait sur
   Lassalle (1,1 %), ce qui suggérait de traiter la position des petits comme
   `dynamic_mask` traite déjà leur `w`. Vraie observation, mauvaise conclusion —
   cf. ci-dessous, en desserrant le lien intra-bloc le pire désaccord passe sur
   **Le Pen (22 %)**.

**Ce qui a permis de trancher : le R-hat PAR SITE.** Le R-hat global est un max
sur tous les paramètres ; il dit qu'il y a un problème, jamais où. Ventilé sur
J-210, les quatre pires sites sont `mu` (2,019), `sd_delta_mu` (1,752),
`mu_slot_pos` (1,549), `mu_base` (1,377) — et `w_now` seulement 1,304. Le
désaccord est **géométrique**, pas dynamique. Ça disculpe d'un coup l'OU,
`sigma_w` et la séparation niveau/dérive, et ça oriente vers la construction des
positions. Mesure ajoutée en dur dans `04k` (`rhat_top`) : elle ne coûte rien et
elle a fait gagner la journée.

**Trois bras sur J-210**, mêmes données, même graine :

| bras | R-hat | ESS | div | pas méd/max | temps | désaccord `mu` méd/max |
|---|---|---|---|---|---|---|
| A0 blocs, `sd_delta_mu = 0,06` | 2,019 | 2,7 | 45 | 511 / 1023 | 1019 s | 0,151 / 0,512 |
| A1 blocs, `sd_delta_mu = 0,25` | 1,719 | 3,5 | 3 | 511 / 1023 | 646 s | 0,089 / 0,574 |
| **A2 blocs CASSÉS** (une position par candidat, ancrée sur son bloc, libre autour) | **1,055** | **68,1** | **0** | **255 / 255** | **271 s** | **0,022 / 0,073** |

Trois choses à retenir.

- **A1 dément un pronostic** qui semblait sûr : desserrer devait aggraver, ça
  améliore nettement (45 divergences → 3). Le `sd_delta_mu = 0,06` créait un
  conflit prior/données franc — six candidats de droite sommés de tenir dans
  ±0,06 alors que les sondages les séparent.
- **A2 casse enfin le mur des 255 pas** poursuivi toute la session (§12.22) :
  profondeur maximale 255 au lieu de 1023, et 3,8× plus rapide. La cause n'était
  donc ni la dimension ni les directions plates, mais bien un **mauvais
  conditionnement**, et il vient de la géométrie des positions.
- **Le suspect est la construction en slots ordonnés elle-même.** `mu_base`
  décale toutes les positions ensemble, `mu_gaps` est une somme cumulée où le
  saut *k* déplace tous les slots ≥ *k* : paramétrisation triangulaire fortement
  corrélée, exactement ce qu'une masse diagonale ne peut pas préconditionner.
  A2 la supprime entièrement.

**Réserve levée en §12.26, et l'hypothèse était fausse** : `B1` impose l'ordre
strict avec une chaîne cumulée de **19** slots et converge très bien (R-hat
1,041, ESS 95). La somme cumulée n'y est donc pour rien. Le fautif est le
**slot PARTAGÉ assorti d'un écart individuel non ordonné** — cf. §12.26.

### 12.26 La cause : un slot PARTAGÉ avec un écart libre — et σ figé est une fausse bonne idée

Cinq bras de plus sur J-210, mêmes données, même graine, pour séparer ce que
§12.25 confondait. « Ordre strict » = un slot par candidat
(`slot_of = arange(N)`, `sd_delta_mu_scale = 0`), l'ordre politique étant une
connaissance qu'on a et qu'on ne donnait pas au modèle. « Ordre mou » = une
ancre par candidat tirée du même ordre, avec liberté ±0,4 en logit autour.

| bras | R-hat | ESS | div | pas méd/max | temps | Zemmour > Le Pen |
|---|---|---|---|---|---|---|
| A0 blocs, `sd_delta_mu = 0,06` | 2,019 | 2,7 | 45 | 511 / 1023 | 1019 s | |
| A1 blocs, `sd_delta_mu = 0,25` | 1,719 | 3,5 | 3 | 511 / 1023 | 646 s | |
| A2 ancre par BLOC, libre autour | 1,055 | 68,1 | 0 | 255 / 255 | 271 s | |
| A3 blocs + OU centré (§12.17) | 1,159 | 13,5 | 62 | 1023 / 1023 | 866 s | |
| B1 ordre STRICT, σ actuel | 1,041 | 95,2 | 1 | 255 / 511 | 444 s | ✓ |
| B2 ordre STRICT, σ unique échantillonné | 1,062 | 86,3 | 0 | 255 / 255 | 286 s | ✓ |
| B3 ordre STRICT, **σ = 0,10 figé** | 1,054 | **4,0** | 0 | 255 / 1023 | 348 s | ✓ |
| **B4 ordre MOU, ancre par CANDIDAT** | **1,011** | **117,3** | 1 | **255 / 255** | **233 s** | ✓ |

**La cause exacte.** Ce n'est ni les blocs comme découpage, ni la chaîne
ordonnée : `B1` empile 19 sauts cumulés et se porte très bien. C'est le **slot
PARTAGÉ assorti d'un écart individuel libre de signe**. Plusieurs candidats sur
une même position `slot_pos[g]`, plus `z_mu · sd_delta_mu` qui peut les
réordonner : le postérieur admet alors les permutations, chaque chaîne en
choisit une, et R-hat mesure ce désaccord (ESS 2,7 = quatre chaînes valant
quatre points). Dès que chaque candidat possède sa propre position — ordonnée
(B1) ou ancrée (A2, B4) — l'ambiguïté disparaît.

C'est aussi la réponse au mur des 255 pas de §12.22 : la trajectoire ne faisait
jamais demi-tour parce qu'elle traversait une crête entre modes permutés.

**σ figé : à ne pas faire, et c'est contre-intuitif.** L'argument semblait
solide — σ n'est identifié par aucune donnée (notice §2, §12.12), donc autant
supprimer ses 26 paramètres. Mesuré, `B3` (σ = 0,10 pour tous) **effondre l'ESS
à 4,0**, contre 95 avec le σ actuel et 86 avec un σ unique échantillonné, et la
profondeur d'arbre repart à 1023. Interprétation : σ ne servait pas qu'à décrire
un rayon, il **absorbait le désajustement** du modèle à une dimension. Le figer
ne supprime pas le désajustement, il le renvoie sur les positions et sur `w`,
qui eux sont contraints. « Non identifié » ne veut donc pas dire « inutile » :
un paramètre peut être indéterminé et néanmoins indispensable à la souplesse du
reste. Le σ unique ÉCHANTILLONNÉ, lui, passe très bien (ESS 86 pour 1 paramètre
au lieu de 26) et reste la simplification recommandée si on en veut une.

**Pourquoi B4 plutôt que l'ordre strict**, alors que la contrainte stricte est
disponible et fonctionne :

1. il gagne sur tous les indicateurs d'échantillonnage, et il est 2× plus rapide
   que B1 ;
2. **il produit l'ordre politique sans l'imposer** — Zemmour 0,943 contre Le Pen
   0,904. L'ordre devient une prédiction vérifiable au lieu d'un postulat, ce
   qui est précisément ce qu'on veut d'un modèle censé situer les candidats ;
3. l'ordre strict sur 19 slots **écrase les extrêmes contre la borne** (Zemmour
   0,989, Le Pen 0,981, indiscernables) : la sigmoïde sur une longue chaîne de
   sauts positifs concentre les derniers slots là où sa dérivée s'annule. C'est
   le défaut de §12.4, réapparu par l'allongement de la chaîne.

L'ordre strict reste disponible (`slot_of = arange(N)` + `sd_delta_mu_scale = 0`)
et il est le bon choix le jour où le postérieur contredirait un ordre qu'on tient
pour acquis.

**Ce qui reste à faire avant d'adopter B4** : la convergence ne suffit pas. La
redistribution hors échantillon est le seul juge de la promesse du §0, et elle
est peu sensible aux problèmes d'échantillonnage — mesurée à **0,559 pt contre
0,699 pt** pour le proportionnel (67 % des paires, 91 paires) sur des fits dont
le R-hat allait de 1,05 à 2,56. Autrement dit, les gains de convergence
ci-dessus ne se liront PAS automatiquement dans ce critère : il faut le remesurer
sous B4, avec la couverture. `04j` a été recâblé pour ça
(`04j_redistribution.py joint`, lit les caches de `04k`).

### 12.27 Le modèle SANS BLOCS — validé sur 2026 avant d'être adopté

Tout ce qui précède a été réglé sur 2021. Le risque de sur-ajustement à cette
seule élection était réel, donc contrôle sur le roster 2027 live, mêmes données,
même graine, **avec la redistribution mesurée à chaque bras** (45 paires
imbriquées, en échantillon — la seule mesure disponible sur 2026, cf. §12.24) :

| 2027 | R-hat | ESS | div | pas | temps | redistribution | inversions d'ordre |
|---|---|---|---|---|---|---|---|
| ACTUEL, blocs partagés | 1,009 | 237,6 | 1 | 255 / 511 | 277 s | 0,766 pt — 82 % | 4 |
| B4, ancre/candidat + σ par bloc | 1,060 | 61,2 | 1 | 127 / 511 | 180 s | 0,782 pt — 80 % | 1 |
| C1, + σ global, jitter 0,15 | 1,014 | 314,1 | **0** | 127 / 255 | 169 s | 0,797 pt — 80 % | 1 |
| **C2, + σ global, jitter 0,30** | **1,013** | **308,6** | **0** | **127 / 255** | **143 s** | **0,762 pt — 82 %** | **aucune** |

**Deux enseignements que le seul 2021 ne donnait pas.**

1. *`B4` seul DÉGRADE 2026* — ESS 238 → 61. Le diagnostic de §12.26 était donc
   incomplet : le défaut n'était pas les ancres, c'était de garder `sigma_slot`
   (par bloc) À CÔTÉ d'ancres par candidat. Deux structures redondantes qui se
   disputent la même information. Une fois les blocs retirés **partout**, tout
   s'aligne.
2. *`C2` bat le modèle actuel sur 2026 aussi*, pas seulement sur 2021 : ESS
   308,6 contre 237,6, zéro divergence, deux fois plus rapide, redistribution
   identique (0,762 contre 0,766, 82 % des paires dans les deux cas). Ce n'est
   donc pas un correctif local à 2021 mais une simplification qui gagne partout.

**Le modèle retenu n'a plus aucun bloc :**

```
mu_i = sigmoid( logit(ancre_i) + z^mu_i · 0,4 )      ancre = rang politique connu
sigma_i = sigma_global · exp( z^sigma_i · 0,30 )     sigma_global échantillonné, prior lâche
w_i(t)  = m_i + sigma_w · (dérive KL tronquée)       statique si part moyenne < 5 %
```

Trois paramètres par candidat, deux échelles globales. Le bloc redevient ce
qu'il aurait dû rester : une **information a priori** servant à placer l'ancre,
et non une variable latente que plusieurs candidats se partagent. C'est le
partage qui cassait l'échantillonnage (§12.26), pas la notion de bloc.

**Le jitter, et pourquoi 0,30 plutôt que 0,15.** Le delta par candidat mesure à
quel point le candidat est un parti *attrape-tout*. À 0,15 la redistribution se
dégrade (0,797 contre 0,762) : la marge est trop étroite pour représenter cette
différence. La largeur est FIXE, et c'est le point — `sd_delta_sigma` était
échantillonné, donc une échelle libre multipliant un vecteur de dimension N,
c'est-à-dire un entonnoir. Fixer la largeur garde un rayon par candidat sans
laisser la dispersion errer. À ne pas confondre avec **figer σ lui-même**, qui
est une très mauvaise idée (§12.26, `B3` : ESS 4,0).

**L'ordre politique devient une PRÉDICTION, pas un postulat.** `C2` ne produit
aucune inversion : le postérieur respecte l'ordre des ancres alors qu'il a ±0,4
en logit pour s'en écarter, largement de quoi faire se croiser deux voisins. Et
l'inverse s'observe aussi — sans aucune ancre, le postérieur 2027 place
spontanément **Dupont-Aignan < Bardella < Le Pen**, contre l'ordre arbitraire
que portait `ORDER_GROUPS` (`Le Pen, Bardella, Dupont-Aignan`). Vérifié comme
conforme à la réalité politique, l'ordre du groupe `MLP` a été corrigé en
conséquence. C'est l'usage recherché : le modèle éclaire le placement au lieu de
le recevoir.

**Ce qui reste à mesurer** : les cinq coupures 2021 sous `C2` (J-150 restait à
R-hat 1,129 / ESS 26,6 sous `B4`), puis couverture et redistribution
hors échantillon sur l'ensemble. Rien n'est adopté avant ça.

### 12.28 Arbitrage sur les cinq coupures — 2021 converge, mais la redistribution régresse

**2021 est débogué.** Les cinq coupures sous `C2`, roster as-of, contre le même
protocole avec blocs partagés :

| coupure | blocs : R-hat / ESS / div | `C2` : R-hat / ESS / div |
|---|---|---|
| J-300 | 1,070 / 51,7 / 10 | **1,013 / 241,1 / 0** |
| J-240 | 1,053 / 76,1 / 10 | **1,013 / 171,8 / 4** |
| J-210 | 2,263 / 2,8 / 80 | **1,037 / 106,3 / 0** |
| J-180 | 2,129 / 2,6 / 14 | **1,066 / 72,6 / 5** |
| J-150 | 2,556 / 2,4 / 1 | **1,055 / 37,7 / 0** |

Plus aucune coupure au-dessus de R-hat 1,07, contre trois au-dessus de 2. Le
« 3 échecs sur 5 » de §12.20, que ce §-là expliquait par un domaine de validité
dépassé, était bien un défaut de spécification — et il est corrigé.

**La couverture ne bouge pas** (même protocole, mêmes fenêtres de test) :

| coupure | blocs IC50/80/90 | `C2` IC50/80/90 |
|---|---|---|
| J-150 | 43,9 / 82,3 / 91,7 | 43,0 / 80,7 / 92,0 |
| J-180 | 46,8 / 81,8 / 91,6 | 47,3 / 82,3 / 91,1 |
| J-210 | 49,2 / 74,2 / 83,2 | 47,5 / 74,6 / 82,0 |
| J-240 | 61,5 / 87,2 / 95,7 | 60,3 / 85,2 / 94,6 |
| J-300 | 50,0 / 73,6 / 81,2 | 50,8 / 74,4 / 82,0 |
| **POOLÉ** | 49,3 / 79,4 / **88,5** | 48,6 / 79,1 / **88,0** |

Deux choses à en tirer, la seconde plus importante que la première.

1. `C2` ne coûte rien en couverture.
2. **La couverture ne discrimine pas la géométrie.** Elle est identique entre un
   modèle convergé (ESS 241) et un modèle qui ne l'était pas (ESS 2,4). Elle est
   donc pilotée par le bruit d'observation et par l'extrapolation de `w`, pas
   par le mécanisme spatial : elle valide la CHAÎNE, pas la promesse du §0. À ne
   plus jamais utiliser seule pour arbitrer un choix de géométrie.

Et l'étalement entre coupures — 82 % à 95,7 % d'IC90 — est **le même dans les
deux modèles**. Le blocage de §12.16 n'est donc imputable ni aux blocs ni à leur
suppression : il reste entier, et il est ailleurs.

**Ce qui bloque l'adoption : la redistribution régresse.**

| | MAE spatial | proportionnel | gain | paires gagnées |
|---|---|---|---|---|
| blocs | **0,559** | 0,699 | +0,140 | **67 %** |
| `C2` | 0,582 | 0,699 | +0,117 | 58 % |

Ce n'est pas du bruit : `C2` est moins bon sur **les cinq** coupures (0,457→0,477,
0,273→0,341, 0,573→0,584, 0,668→0,693, 0,504→0,534).

C'est précisément le piège annoncé en §12.26 : *un modèle qui converge parce
qu'on lui a retiré de la souplesse n'est pas une amélioration.* La convergence
est spectaculaire, la promesse du modèle recule ; `C2` n'est donc PAS adopté en
l'état.

**Cause la plus probable, et elle est corrigeable.** Les ancres sont
**équiréparties** (`rang/(N+1)`), ce qui affirme que les candidats sont
politiquement équidistants. C'est faux, et la redistribution dépend exactement
des écarts relatifs — le softmax masqué envoie les électeurs d'un candidat
retiré vers ses VOISINS, donc l'espacement est le mécanisme, pas un détail. Le
modèle à blocs, lui, plaçait les positions là où les données les voulaient, au
prix de la multimodalité. `pos_affine` (§12.27) ne peut pas corriger ça : il
n'autorise qu'un étirement GLOBAL, et il mesure d'ailleurs `pos_stretch = 1,095`,
soit « l'échelle d'ensemble est déjà bonne ». Ce qui manque est un espacement
NON UNIFORME, c'est-à-dire de la liberté individuelle : `pos_scale`.

### 12.29 Décision — `C2` adopté, et ce qu'il reste à payer

**L'écart de redistribution est réel.** Comparaison APPARIÉE (mêmes 91 paires,
mêmes coupures), bootstrap 20 000 tirages :

| | MAE spatial | paires gagnées |
|---|---|---|
| blocs partagés | 0,5591 | 67 % |
| `C2`, `pos_scale = 0,7` | 0,5774 | 62 % |

Écart apparié **+0,0183 pt, IC95 [+0,0068 ; +0,0295]**, P(C2 pire) = 0,999.
Comparer deux moyennes non appariées aurait conclu « pas significatif »
(SE ≈ 0,042) : l'écart-type des différences PAR PAIRE vaut 0,055, donc
l'appariement est ce qui rend la question décidable. À refaire ainsi
systématiquement.

Les deux battent nettement le proportionnel — blocs −0,1395 pt
[−0,204 ; −0,081], `C2` −0,1213 pt [−0,187 ; −0,062]. `C2` conserve donc ~87 %
de l'avantage.

**Deux explications faciles, écartées.**

1. *Un ensemble accidentel.* Le modèle à blocs ne converge pas (ESS 2,4) : sa
   prédiction moyenne quatre modes distincts, ce qui pourrait le favoriser
   artificiellement. Mesuré en évaluant CHAQUE chaîne séparément : le gain
   d'agrégation ne vaut que **+0,0044 pt**, et chaque chaîne isolée (0,5572 à
   0,5687) bat encore `C2` (0,5738 à 0,5822). L'avantage est structurel.
   *Au passage* : les quatre modes redistribuent quasi identiquement — ils
   diffèrent en paramètres, pas en prédiction, ce qui est exactement pourquoi
   les données ne les départagent pas.
2. *C'est σ qui coûte la redistribution.* Décomposition 2×2 sur J-210 :

| | redistribution | R-hat / ESS |
|---|---|---|
| positions BLOC + rayon BLOC | **0,541** (86 %) | 2,753 / **2,3** |
| positions BLOC + rayon `C2` | 0,659 (71 %) | 1,148 / 13,5 |
| positions ANCRE + rayon BLOC | 0,602 (68 %) | 1,169 / 24,0 |
| positions ANCRE + rayon `C2` | 0,582 (79 %) | 1,053 / 98,3 |
| positions ANCRE + `jitter = 0,60` | 0,622 (64 %) | 1,329 / 5,6 |

   Les effets ne sont **pas additifs** : changer un seul facteur dégrade (0,541
   → 0,659 ou 0,602), changer les deux donne 0,582, mieux que chaque changement
   isolé. Positions et rayon doivent s'accorder ; il n'y a pas de facteur
   coupable à isoler. Et `jitter = 0,60` dégrade — plus de liberté sur σ n'aide
   pas, 0,30 est le bon point.

**Décision : `C2` est adopté**, avec `pos_scale = 0,7` (balayage 0,4/0,7/1,0/1,4 :
0,7 gagne sur la MAE et sur le taux de paires, 68 % → 79 % sur J-210).

Le motif n'est pas seulement qu'un postérieur que quatre chaînes ne partagent
pas ne permette **ni intervalle de crédibilité honnête, ni scénario utilisateur
fiable, ni mise en production** — c'est que **la régression de 0,018 pt ne coûte
rien là où le modèle sert**. Cf. §12.30 : ventilé par difficulté, l'écart tombe
à +0,0010 pt [−0,021 ; +0,024] sur le quartile de paires que le proportionnel
rate, et s'y concentre entièrement sur les paires faciles. Il n'y a donc pas de
prix à payer, et la formulation « 0,018 pt est le prix de la validité »,
écrite ici dans un premier temps, était fausse — elle agrégeait des paires
hétérogènes.

**Ce qu'il reste à payer, et par où le récupérer.** L'avantage des blocs vient
très probablement de ce que leurs positions étaient **inférées** — le modèle
apprenait où se situe « la droite » — alors que mes ancres affirment que les
candidats sont ÉQUIDISTANTS (`rang/(N+1)`). Or l'espacement EST le mécanisme de
redistribution : le softmax masqué envoie les électeurs d'un candidat retiré
vers ses voisins. `pos_affine` ne peut pas corriger ça (il n'autorise qu'un
étirement global, et mesure `pos_stretch = 1,095`, soit « l'échelle d'ensemble
est déjà bonne »). Ce qui manque est un espacement **non uniforme**, et la
liberté individuelle ne le reconstitue qu'en partie (`pos_scale = 1,4` dégrade).

La bonne piste est donc **de meilleures ancres**, pas plus de liberté autour de
mauvaises ancres : placement expert, ou une source externe de positions
partisanes. C'est aussi la seule qui serve l'objectif affiché — moins
d'hypothèses arbitraires, pas moins d'hypothèses tout court : `rang/(N+1)` EST
une hypothèse forte, simplement invisible.

**Note de portée** : cette régression est spécifique à 2021. Sur 2027, `C2`
égale le modèle à blocs en redistribution (0,762 contre 0,766 pt, 82 % des
paires dans les deux cas) tout en convergeant mieux. Les blocs de 2021 étaient
chargés (jusqu'à 6 candidats), donc leurs positions inférées portaient beaucoup
d'information ; ceux de 2027 en portent peu.

### 12.30 Le gain de redistribution suit la DIFFICULTÉ, pas la taille de l'échantillon

Question utilisateur : J-240 est-elle une coupure trop pauvre en données ?
Réponse mesurée : non. Le gain du spatial suit la difficulté du problème, pas le
volume d'entraînement.

| coupure | nœuds train | proportionnel | spatial | gain | paires « faciles » (prop < 0,5 pt) |
|---|---|---|---|---|---|
| J-300 | 36 | 0,533 | 0,513 | +0,02 | 50 % |
| J-240 | 56 | 0,632 | 0,688 | **−0,06** | 38 % |
| J-210 | 86 | 0,785 | 0,582 | +0,20 | 25 % |
| J-180 | 151 | **1,391** | 0,335 | **+1,06** | **0 %** |
| J-150 | 195 | 0,640 | 0,477 | +0,16 | 44 % |

Par PAIRE, sur les 91 : `corr(erreur du proportionnel, gain du spatial) = +0,742`.
Ventilé par quartile de difficulté :

| quartile de `prop` | n | prop | spatial | gain | gagnées |
|---|---|---|---|---|---|
| 0,10 – 0,44 | 23 | 0,340 | 0,374 | −0,034 | 43 % |
| 0,44 – 0,65 | 22 | 0,519 | 0,541 | −0,022 | 45 % |
| 0,65 – 0,81 | 23 | 0,701 | 0,664 | +0,037 | 61 % |
| **0,81 – 1,55** | 23 | 1,227 | **0,728** | **+0,498** | **96 %** |

C'est le comportement qu'on veut : sur les reports réellement NON
proportionnels, le modèle gagne 96 % des paires et divise l'erreur par 1,7 ; sur
les reports proportionnels, il est neutre à légèrement négatif — il n'a rien à
ajouter et n'ajoute que du bruit. J-240 n'est donc pas défaillante, ses paires de
test sont simplement faciles à 38 %.

**Et c'est ce qui tranche définitivement §12.29.** Le même découpage appliqué à
la comparaison blocs / `C2` :

| quartile | n | blocs | `C2` | écart |
|---|---|---|---|---|
| 0,10 – 0,44 | 23 | 0,353 | 0,374 | +0,021 |
| 0,44 – 0,65 | 22 | 0,509 | 0,541 | +0,032 |
| 0,65 – 0,81 | 23 | 0,645 | 0,664 | +0,019 |
| **0,81 – 1,55** | 23 | **0,727** | **0,728** | **+0,001** [−0,021 ; +0,024] |

Sur le quartile qui compte, les deux modèles sont **indiscernables** et gagnent
tous deux 96 % des paires. La régression de `C2` est entièrement portée par les
paires que le proportionnel traite déjà bien.

**Leçon de méthode, la troisième fois aujourd'hui** : un critère agrégé masque
des compensations. La MAE poolée mélangeait des paires où le modèle est décisif
et des paires où il ne peut rien. Le bon résumé de la promesse du §0 n'est pas
« 0,577 pt contre 0,699 » mais **« sur les reports non proportionnels, 0,73 pt
contre 1,23, et 96 % des paires gagnées »**. À utiliser tel quel sur le site :
c'est plus honnête et c'est plus fort.
