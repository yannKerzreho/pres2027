...
def filter_presidential(df: pd.DataFrame, since_year: int = 2025) -> pd.DataFrame:
    ...
    Filtre par MOT-CLÉ dans `name` — marche pour le cycle 2027 (notices
    nommées « Pres ... ») mais PAS pour les archives 2021/2022 : leur
    convention de nommage ne contient presque jamais « pres »/« présid »
    (vérifié : 5/400 notices 2022 matchées). Pour ces cycles plus anciens,
    utiliser `filter_by_date_range` à la place."""
...
def filter_by_date_range(df: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    """Notices dont `pdf_creation_date` tombe dans [start, end] (« AAAA-MM-JJ »).

    Repli pour les cycles électoraux plus anciens (2016-2017, 2021-2022) dont
    le nommage ne permet pas un filtrage par mot-clé fiable (cf.
    `filter_presidential`). Sur la fenêtre resserrée d'une campagne
    présidentielle, la quasi-totalité des sondages publiés en France
    concernent la présidentielle — un filtrage temporel suffit, sans
    prétendre à la même précision que le filtre par mot-clé du cycle 2027."""
    dates = pd.to_datetime(df["pdf_creation_date"], errors="coerce", utc=True)
