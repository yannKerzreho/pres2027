...
# `movement_pool` / `institut_bias_prior` vivent dans `movements.py` : le jeu de
# données observé est partagé, la LOI qu'on y ajuste ne l'est pas. Ce module
# n'expose que le fit sinh-arcsinh utilisé par `bayesian_nowcast`.
from model.core.movements import institut_bias_prior, movement_pool  # noqa: F401
